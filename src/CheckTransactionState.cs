...
// Checks the status of a transaction until completion
public static class CheckTransactionState
{
	/// <summary>
	/// Checks the status of a transaction until completion
	/// </summary>
	...
	public static async Task Run(PhantasmaAPI api, string txHash, Action<ExecutionState?, string?>? onDone)
	{
				...
				else
				{
					...
					switch (tx.State)
					{
						...
						case ExecutionState.Halt:
							...
							onDone?.Invoke(tx.State, tx.Result);
							continue;
